...
function App() {
  ...
  useEffect(() => {
    window.__dcStartHandoff = () => {
      ...
      // Phase 2: morph nodes (strip text, stack center)
      setTimeout(() => setHandoffPhase("morph-nodes"), 2000);
      // Phase 3: long pause on stripped canvas so we can review/edit
      // before the handoff screen appears — increase this delay when ready
      setTimeout(() => setHandoffPhase("handoff-screen"), 30000);
      // Navigate away
      setTimeout(() => {
        window.location.href = "StageTransition.html?from=catcher&to=studio";
      }, 8000);
    };

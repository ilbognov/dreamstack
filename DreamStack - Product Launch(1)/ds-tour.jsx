...
(function () {
  ...
  const STOPS = [
    ...
    },
    {
      ...
      highlightPad: 0,
      highlightCropBottom: 260, // trim bottom of spotlight — dim toolbar & tip area
    ...
    },
    {
      ...
      highlightPad: 6,
      anchor: "left",
      title: "The AI coaches, you commit."move triage before upload.\" The AI proposes edits; nothing hits the architecture until you confirm. This is your seat. No one else is here.",
    },
    {
      id: "push",
      requireRoute: "session",
  ...
  function useRectOf(sel, activeStep) {
    ...
    useEffect(() => {
      ...
      const measure = () => {
        ...
        // We need coordinates relative to #ds-frame (the 1440x872 surface).
        const frame = document.getElementById("ds-frame");
        if (!frame) { setRect(null); return; }
        const fr = frame.getBoundingClientRect();
        // Scale: convert screen px to frame px.
  ...
  function TourOverlay({ onFinish, onPushStep, route }) {
    ...
    // Effective spotlight rect (after cropping bottom).
    const effRect = rect ? {
      left:   rect.left,
      top:    rect.top,
      width:  rect.width,
      height: Math.max(60, rect.height - cropBottom),
    } : null;
    ...
    if (!stop) return null;
    // Don't render if the stop is for a different route than we're currently in.
    // This lets the tour quietly wait through route transitions.
    if (stop.requireRoute && route && stop.requireRoute !== route) return null;
    // Breathe: after arriving in session from the home stop, hold the whole
    // overlay off for a beat so the user sees the full dashboard.
    ...
    return (
      <div className="ds-tour-root">
        ...
        {rect && !stop.centered ? (
          ...
          !cursorRunning && <div className="ds-tour-dim"/>
        )}

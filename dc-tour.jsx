...
function Tour({ cursor, playing, setPlaying, messages, nodes, pending = [], asks, setTab, openNodeQuestions, closeNodeQuestions, enabled = true }) {
  ...
  // Expose a jump-to-step API so external triggers (e.g. the chrome info
  // icon) can fast-forward straight into a specific tour stop without
  // playing through the demo. 1-indexed to match the visible badge.
  React.useEffect(() => {
    window.__dcTourJumpToStep = (n) => {
      const idx = Math.max(1, Math.min(TOUR_STOPS.length, n)) - 1;
      const stop = TOUR_STOPS[idx];
      if (!stop) return;
      // Mark all stops up to & including the target as fired so the
      // "N of M" badge and back-chevron history are coherent.
      const priorIds = TOUR_STOPS.slice(0, idx + 1).map(s => s.id);
      setSkipped(false);
      setPausedStopId(null);
      setFiredIds(new Set(priorIds));
      setStopHistory(priorIds);
      setPlaying(false);
      stop.sideEffect?.({ setTab, asks, nodes, openNodeQuestions, closeNodeQuestions });
      setActiveStop(stop);
    };
    return () => { delete window.__dcTourJumpToStep; };
  }, [setTab, asks, nodes, openNodeQuestions, closeNodeQuestions, setPlaying]);
